...
function snowcollect()
{
    ...
    {
        if ($api->UserInfoGet($userid, 'energy', true) < 10)
        {
            alert('danger',"Uh Oh!","You do not have enough energy to attempt to collect snow. You must have at least 10%.",true,'explore.php');
            die($h->endpage());
        }
        if ($api->UserInfoGet($userid, 'brave', true) < 2)
        {
            alert('danger',"Uh Oh!","You do not have enough brave to attempt to collect snow. You must have at least 2%.",true,'explore.php');
            die($h->endpage());
        }
        if ($api->UserInfoGet($userid, 'will', true) < 1)
        {
            alert('danger',"Uh Oh!","You do not have enough will to attempt to collect snow. You must have at least 1%.",true,'explore.php');
            die($h->endpage());
        }
        $api->UserInfoSet($userid,'will',-1,true);
        $api->UserInfoSet($userid,'brave',-2,true);
        ...
        if ($chance == 3)
        {
            alert('danger',"Uh Oh!","You wanted to find snow, but couldn't. Or rather, you walked right back into the warmth as soon as you stepped outside.",true,'explore.php');
            die($h->endpage());
        }
    }
    else
    {
        echo "Would you like to collect some snow? It requires 10% energy, 1% Will and 2% Brave.<br />
        <a class='btn btn-primary' href='?action=snowcollect&collect=true'>Collect Snow</a>";
    }
    $h->endpage();
}

function cleanadminstuff()
{
    global $db, $userid, $h;
    if ($userid != 1)
    {
        alert('danger',"Uh Oh!","Admins only.", true, 'explore.php');
        die($h->endpage());
    }
    $db->query("TRUNCATE `2018_christmas_tree`");
    $db->query("TRUNCATE `2018_christmas_wishes`");
    $db->query("TRUNCATE `advent_calendar`");
    alert('success',"Success!","Christmas database tables are cleared.", true, 'explore.php');
    die($h->endpage());
}