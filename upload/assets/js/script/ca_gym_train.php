...
{
    alert("danger", "Uh Oh!", "You need at least one CID Admin Gym Access Scroll to use the CID Admin's Personal Gym.", false);
    die($h->endpage());
}
...
if (isset($_POST["stat"]) && $_POST["amnt"]) 
{
	...
	else
	{
        ...
        if (userHasEffect($userid, effect_daily_gym_bonus))
            $multi = $multi + ((returnEffectMultiplier($userid, effect_daily_gym_bonus) / 100) * 8);
        if ($stat == 'all') 
        {
        ...
        elseif ($stat == "labor") {
            alert('success', "Success!", "You begin moving boxes around the gym. You have gained " . shortNumberParse($gain) . " Labor by moving
                " . shortNumberParse($_POST['amnt']) . " sets of boxes. You now have " . shortNumberParse($NewStatAmount) . " and {$EnergyLeft} Energy left.", false);
            //Have guard selected for the next training.
            $lab_select = "selected";
        ...
        $api->SystemLogsAdd($userid, 'training', "[CID Admin Gym] " . shortNumberParse($_POST['amnt']) . " energy for " . shortNumberParse($gain) . " {$stat}.");
        echo "<hr />";
