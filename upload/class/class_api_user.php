    ...
    */
	function setInfo(int $user, string $stat, int $change)
	{
		...
		else
		{
			...
			// Handle special cases for max columns that have different names
			if ($stat == 'xp') {
				// XP doesn't have a max cap, it just accumulates
			} else {
				// For other stats, cap at max value if column exists
				$max_col = "max{$stat}";
				$check_query = $db->query("SHOW COLUMNS FROM users LIKE '{$max_col}'");
				if ($db->num_rows($check_query) > 0) {
					$db->query("UPDATE users SET `{$stat}` = `{$max_col}` WHERE `{$stat}` > `{$max_col}` AND `userid` = {$user}");
				}
