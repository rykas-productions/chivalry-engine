...
function set_town_gym_percents()
{
    global $db;
    $q = $db->query("SELECT * FROM `town`");
    while ($r = $db->fetch_row($q)) {
        $newGymPerc = Random(95, 111);
        $db->query("UPDATE `town` SET `town_gym_percent` = '{$newGymPerc}' WHERE `town_id` = {$r['town_id']}");
    }
}
