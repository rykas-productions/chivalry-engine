...
if ($ir['guild'] > 0) {
	...
	if ($db->num_rows($distQ) == 0) {
		$db->query("INSERT INTO `guild_district_info` 
					(`guild_id`, `barracks_warriors`, `barracks_archers`, `barracks_generals`, `moves`, `barracks_captains`) 
					VALUES 
					('{$ir['guild']}', '0', '0', '0', '2', '0')");
	}
		...
		while ($r = $db->fetch_row($q)) {
			...
			echo "<div class='col-12 col-sm-6 col-md-4 col-xl'>
                    <div class='card {$class} text-muted'>
                        <div class='card-header'>
                            Y: {$r['district_y']}; X: {$r['district_x']} " . returnTileIcon($r['district_id']) . "
                        </div>
                        <div class='card-body'>
                            <div class='row'>
                                <div class='col-auto'>
...
function attackfromtile()
{
	...
	if (isset($_POST['warriors'])) {
		...
		$i = newLogBattle(
			...
			$warriors,
			$archers,
			$results['attack_warrior_lost'],
			$results['attack_archer_lost'],
			$captains,
			$results['attack_luck'],
			$r2['district_melee'],
			$r2['district_range'],
			$results['defense_warrior_lost'],
			$results['defense_archer_lost'],
			$r2['district_general'],
			$r2['district_fortify'],
			$results['defense_luck']
		);
