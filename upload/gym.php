...
$townMulti = $db->fetch_single($db->query("SELECT `town_gym_percent` FROM `town` WHERE `town_id` = {$ir['location']}"));
$multi *= ($townMulti / 100);
$_GET["amnt"] = abs($_GET["amnt"]);
echo "<h3>🎽 {$ir['town_name']} Gym</h3>";
...
At the {$ir['town_name']} Gym, you'll earn stats at " . number_format($multi * 100) . "% efficiency.
