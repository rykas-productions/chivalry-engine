...
if (isset($_GET["stat"]) && $_GET["amnt"]) {
    ...
    if ($_GET['amnt'] > $ir['energy']) {
        ...
        $extraecho = '';
        if ($stat == 'all') {
            $gainstr = $api->UserTrain($userid, 'strength', $_GET['amnt'] / 4);
            $gainagl = $api->UserTrain($userid, 'agility', $_GET['amnt'] / 4);
            $gaingrd = $api->UserTrain($userid, 'guard', $_GET['amnt'] / 4);
            $gainlab = $api->UserTrain($userid, 'labor', $_GET['amnt'] / 4);
        } else {
            $gain = $api->UserTrain($userid, $_GET['stat'], $_GET['amnt']);
        }
