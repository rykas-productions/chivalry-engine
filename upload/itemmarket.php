...
function buy()
{
	...
	else
	{
		$q = $db->query("SELECT `imADDER`, `imCURRENCY`, `imPRICE`, `imQTY`,
			...
			event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a> bought {$_POST['QTY']} {$r['itmname']}(s) from the market for " . number_format($final_price) . " {$curre}.");
...
function gift()
{
	...
	elseif (!empty($_POST['user']))
	{
		...
		$db->query("UPDATE `users` SET `{$curr}` = `{$curr}` - {$final_price} WHERE `userid`= {$userid}");
		$db->query("UPDATE `users` SET `{$curr}` = `{$curr}` + {$final_price} WHERE `userid` = {$r['imADDER']}");
		event_add($_POST['user'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a> bought you {$_POST['QTY']} {$r['itmname']}(s) from the market.");
		event_add($r['imADDER'], "<a href='viewuser.php?u=$userid'>{$ir['username']}</a> bought {$_POST['QTY']} {$r['itmname']}(s) from the market for " . number_format($final_price) . " {$curre}.");
