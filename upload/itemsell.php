...
// Sanitize GET and POST parameters (numeric values)
    ...
    if (!empty($_POST['qty']) && !empty($_GET['ID'])) {
        ...
        if ($db->num_rows($id) == 0) {
            ...
            if ($_POST['qty'] > $r['inv_qty']) {
                ...
                alert('success', "Success!", "You have successfully sold " . shortNumberParse($_POST['qty']) . " {$r['itmname']}(s) back to the game for {$priceh} Copper Coins.", true, 'inventory.php');
        ...
        if ($db->num_rows($id) == 0) {
            ...
            $itmNameEsc  = htmlspecialchars($r['itmname'], ENT_QUOTES, 'UTF-8');
            $invQty      = (int)$r['inv_qty'];
            $csrfCodeEsc = htmlspecialchars($code, ENT_QUOTES, 'UTF-8');
            
            echo "<form id='sellItemForm' action='?ID={$_GET['ID']}' method='post'>
                <div class='card'>
                    <div class='card-header'>
                        Selling your {$itmNameEsc}(s)
                    </div>
                    <div class='card-body'>
                        <div class='row'>
                            <div class='col-12'>
                                You may sell your " . shortNumberParse($invQty) . " {$itmNameEsc}(s) back to the game for their sell price.
                            </div>
                            <div class='col-auto'>
                                <div class='row'>
                                    <div class='col-12'>
                                    ...
                                    <div class='col-12'>
                                        <input type='text' class='form-control' name='qty' value='{$invQty}' max='{$invQty}' />
                                    </div>
                                </div>
                            </div>
                            <div class='col-auto'>
                ...
                <input type='hidden' name='verf' value='{$csrfCodeEsc}' />
            </form>";
            ...
            echo "<script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    form.addEventListener('submit', function (e) {
                        var confirmSell = confirm('Are you sure you want to sell this item? This action cannot be undone.');
                        if (!confirmSell) {
