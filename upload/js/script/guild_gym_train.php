	...
	else
	{
        ...
        elseif ($stat == "labor") {
            alert('success', "Success!", "You begin moving boxes around the gym. You have gained " . shortNumberParse($gain) . " Labor by moving
                {$_POST['amnt']} sets of boxes. You now have " . shortNumberParse($NewStatAmount) . " and {$EnergyLeft} Energy left.", false);
            //Have guard selected for the next training.
            $lab_select = "selected";
        ...
        $api->SystemLogsAdd($userid, 'training', "[Guild Gym] " . shortNumberParse($_POST['amnt']) . " energy for " . shortNumberParse($gain) . " {$stat}.");
        echo "<hr />";
