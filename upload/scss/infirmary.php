    ...
    if (isset($_GET['user'])) {
        ...
        if (isset($_GET['times'])) {
            ...
            if ($ir['secondary_currency'] < $cost) {
                alert('danger', "Uh Oh!", "You do not have enough Chivalry Tokens to heal " . number_format($_GET['times']) . " sets. You need " . number_format($cost) . ", but only have " . number_format($ir['secondary_currency']) . " Chivalry Tokens.", true, 'infirmary.php');
                die($h->endpage());
            } else {
                //Healed successfully!
                ...
                //Add a friendly note.
                $api->GameAddNotification($_GET['user'], "<a href='profile.php?user={$userid}'>{$ir['username']}</a> has healed you " . number_format($_GET['times']) . " times.");
                //Log it!
                $api->SystemLogsAdd($userid, 'heal', "Healed {$api->SystemUserIDtoName($_GET['user'])} " . number_format($_GET['times']) . " times.");
                alert('success', "Success!", "You have healed {$api->SystemUserIDtoName($_GET['user'])} " . number_format($_GET['times']) . "
                times, costing you " . number_format($cost) . " Chivalry Tokens.", true, 'index.php');
            }
        } else {
