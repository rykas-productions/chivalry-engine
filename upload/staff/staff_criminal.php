...
function new_crime()
{
    ...
    if (!isset($_POST['name'])) {
        ...
        $_POST['percform'] = (isset($_POST['percform']) && preg_match("/^[a-z0-9\p{Sm}\s()*.\/-_]+([\\s]{1}[a-z0-9\p{Sm}\s()*.\/-_]|[a-z0-9\p{Sm}\s()*.\/-_])*$", $_POST['percform'])) ? $db->escape(strip_tags(stripslashes($_POST['percform']))) : '';
